...
static void calculate_aa_chirality(AA *a) {
	...
	if (p<2.0) {
	   fprintf(stderr, "calc_chiral %s %5d : dist(ca-cb) %g\n", aa123(a->id), a->num, sqrt(p));
	   exit(1);
	}
...
void acidate(AA *a, triplet xnt, triplet xct, simulation_params *sim_params)
{
	...
	if (a->id != 'G') {
		...
		/*   fprintf(stderr,"ca-cb distance2 = %g\n", (a->cb[0]-a->ca[0])*(a->cb[0]-a->ca[0]) + */
		/* 	  (a->cb[1]-a->ca[1])*(a->cb[1]-a->ca[1]) + */
		/* 	  (a->cb[2]-a->ca[2])*(a->cb[2]-a->ca[2])); */
		/* } */
...
void aat_init(Chain * chain, Chaint * chaint){
  if(sizeof(chaint)->aat != chain->NAA * sizeof(AA)){
    ...
    for (i = 1; i < chain->NAA; i++) {
	  ...
	  (chaint)->aat[i].sideChainTemplateIndex = chain->aa[i].sideChainTemplateIndex;
    }
  }	
...
//fprintf(stdout,"FULFILL CALLED\n");
	...
	for (i = 1; i < chain->NAA; i++)
	  // MS added chain->aa[i-1].chainid>0 to avoid entering if when we have only one chain
	  if ((chain->aa[i].chainid != chain->aa[i-1].chainid) && (chain->aa[i-1].chainid>0) ) {
...
void build_peptide_from_sequence(Chain * chain, Chaint *chaint, char *str, simulation_params *sim_params)
{
	...
	/* MS amino acid codes can be prefixed with & to make them D-amino acids
	   the amino acid code can also be followed by \name\. When this is the case the coarse potential of
	   of the amino acid is used for the peptide internal energy and the side chain will be built and
	   evaluated using the "name" sidechain template which has to be present in canonicalAA _AASCRotTable
	 */
	...
	char ** SCTnames = (char **)malloc((strlen(str)+1)*sizeof(char *));
	int * dAA = (int *)malloc((strlen(str)+1)*sizeof(int));
	int * chain_ids = (int *)malloc((strlen(str)+2)*sizeof(int));
	int next = 0;
	for (next=0; next<strlen(str)+1; next++) {
	  SCTnames[next] = NULL;
	...
	for (int j = 0; str[j] != '\0'; j++){
	    if (str[j]=='&') { /* this AA has to be DEXTRO */
	        ...
	        int count = 0;
		while (1) {
		    if (str[j+count]=='>') break;
		    if (str[j+count]=='\0') stop("end of name '>' missing");
		    count++;
		}
		SCTnames[next-1] = (char*)malloc(count);
		memcpy(SCTnames[next-1], &str[j+1], count-1);
		SCTnames[next-1][count-1] = '\0';
		j += count;
	    } else if (str[j]=='_') { /* separator means new chain */
	        next_chain_id ++;
	    } else {  /* copy amino acid characters */
...
int fullAApdbrecord( AA *a, int j, model_params *mod_params, FILE *outfile)
{
  int i, nbAtoms, len;
    ...
    if (a->sideChainTemplateIndex>=0) {
	  ...
	  atname[4] = '\0';
	  //printf("%s|",atname);
	  fprintf(outfile, fmt, ++j, atname, coords[i][0], coords[i][1], coords[i][2]);
	}
    }
    //printf("\n");
    return j;
...
void pdbprint( AA *a, int count, model_params *mod_params, FILE *outfile, double *E_tot)
{
	...
	for (j = 0, i = 1; i < count; i++) {
		...
		j = fullAApdbrecord(a + i, j, mod_params, outfile);
	}
...
int i,j;
  ...
  for(j = 0; j < to->NAA; j++){
    ...
    to->aa[j].sideChainTemplateIndex = from->aa[j].sideChainTemplateIndex;
    for(i = 0; i < 3; i++){
